...
uint64_t Tools::buildLong(uint8_t bytes[LONGSIZE])
{
    uint64_t result = 0;
    for(int x=7; x>0; --x) {
        result = result | (uint64_t)bytes[x];
        result = result << 8;
...
{
    if(low < 0 || high > 63 || low > 63 || high < 0) return source;
    else
    {
        uint64_t mask = 0xffffffffffffffff;
        mask = mask << (63 - high);
...
{
    int64_t high = (srclow + (length - 1));

    if((srclow < 0 ) || (dstlow < 0) || (high > 63 ) || (length - 1) < 0) return dest; 
    else {

        uint64_t storage = dest;
        uint64_t source2 = getBits(source, srclow, high);

        source2 = source2 << dstlow;
        uint64_t destHigh = (dstlow + (length - 1));
        dest = clearBits(dest, dstlow, destHigh);
        dest = dest | source2;
        if (destHigh > 63) {
            dest = storage; 
            return dest;
...
{
    uint64_t mask = 0xff;
    uint64_t check = ((int64_t)(!(byteNum >> 3)) << 63) >> 63;
    mask = mask << (byteNum * 8);

    return ((mask | source) & check) | (source & ~check); 
}
...
{
    uint64_t combo = op1 + op2;
    uint64_t sign1 = sign(op1);
    uint64_t sign2 = sign(op2);
    uint64_t comboSign = sign(combo);

    return (sign1 == sign2) && (comboSign != sign1);
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
    ...
    uint64_t negOp2 = (~op2 + 1);
    uint64_t combo = op1 + negOp2;
    uint64_t sign1 = sign(op1);
    uint64_t sign2 = sign(negOp2);
    uint64_t comboSign = sign(combo);
