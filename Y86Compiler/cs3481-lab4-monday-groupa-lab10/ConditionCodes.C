...
//Anthony Correale & Logan Richardson
...
{
    if(!ccInstance) ccInstance = new ConditionCodes;
    return ccInstance;
}
...
bool ConditionCodes::getConditionCode(int32_t ccNum, bool & error)
{
    ...
    uint64_t zeValue = Tools::getBits(codes, ccNum, ccNum); 
    error = false;
        ...
        bool & error)
{
    ...
    if (!value) codes = Tools::clearBits(codes, ccNum, ccNum);
    else codes = Tools::setBits(codes, ccNum, ccNum);
